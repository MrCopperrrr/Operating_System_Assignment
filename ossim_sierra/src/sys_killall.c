...
int __sys_killall(struct pcb_t *caller, struct sc_regs* regs)
{
    ...
    do {
        ...
        proc_name[i] = (data == -1) ? '\0' : (char)data;
        i++;
    } while (data != -1 && i < 100);

    printf("The procname retrieved from memregionid %d is \"%s\"\n", memrg, proc_name);

    // Hàm tự viết inline: lặp qua queue, kill những proc có tên trùng
    // Viết 2 lần, 1 cho từng queue
    ...
    if (q) {
        ...
        for (int i = 0; i < old_size; ++i) {
            ...
            if (proc && strcmp(proc->path, proc_name) == 0) {
                printf("[KILLALL] Killed process \"%s\" (pid: %d)\n", proc->path, proc->pid);
                // Không enqueue lại
            } else {
                enqueue(q, proc);
            }
